import { 
  ...
  Lock, History, Save, LogOut, Camera, CheckCircle2
} from "lucide-react";
import { useState } from "react";
import { useLogout } from "./AdminLayout";
  ...
  return (
    <div className="pb-12 flex flex-col space-y-10 animate-in fade-in duration-500 font-poppins">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-2">Profil Saya</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-900/5 p-8 text-center relative overflow-hidden group">
              ...
              <div className="mb-8">
                 <h3 className="text-xl font-black text-slate-900 tracking-tight">Admin Utama</h3>
                 <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1 bg-blue-50 inline-block px-3 py-1 rounded-lg">Super Admin</p>
              </div>
              ...
              <div className="space-y-3 pt-6 border-t border-slate-50">
                 <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl text-left">
                    ...
                    <div>
                       ...
                       <p className="text-xs font-black text-slate-900 uppercase">Terverifikasi</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl text-left">
                    <History className="w-5 h-5 text-indigo-500" />
                    <div>
                       <p className="text-[9px] font-black text-slate-400 uppercase">Bergabung Sejak</p>
                       <p className="text-xs font-black text-slate-900 uppercase">Januari 2026</p>
                    </div>
                 </div>
              </div>
           </div>
           ...
           {activeTab === 'info' && (
              <div className="p-10 animate-in slide-in-from-right-4 duration-300">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2">
                       ...
                       <div className="relative">
                          ...
                          <input 
                            type="email" 
                            defaultValue="admin@singgalangjaya.com"
                            className="w-full pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-xs font-black text-slate-900 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none"
                          />
                       </div>
                    </div>
                    ...
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Lokasi Penugasan</label>
                       <div className="relative">
                          <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                          <input 
                            type="text" 
                            defaultValue="Padang Panjang, Sumatera Barat"
                            className="w-full pl-12 pr-6 py-4 bg-slate-50 border-none rounded-2xl text-xs font-black text-slate-900 focus:ring-4 focus:ring-blue-600/5 transition-all outline-none"
                          />
                       </div>
                    </div>
                 ...
                 <div className="mt-12 p-6 bg-amber-50 rounded-[2rem] border border-amber-100 flex items-start gap-4">
                    <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center text-amber-600 shadow-sm shrink-0">
                       <ShieldCheck className="w-5 h-5" />
                    </div>
                    <div>
                       <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">Peringatan Keamanan</p>
                       <p className="text-[11px] font-bold text-amber-800/70 leading-relaxed">Informasi profil ini bersifat sensitif. Pastikan data yang Anda masukkan benar karena akan muncul di manifes dan laporan sistem.</p>
                    </div>
                 </div>
              </div>
           )}
           ...
           {activeTab === 'activity' && (
              <div className="p-10 animate-in slide-in-from-right-4 duration-300">
                 <div className="flex items-center gap-3 mb-10 pb-6 border-b border-slate-50">
                    ...
                    <h4 className="text-lg font-black text-slate-900 uppercase tracking-widest">Riwayat Aktivitas Login</h4>
                 </div>

                 <div className="space-y-4">
                    {[
                       { date: "Hari Ini, 14:20", device: "Chrome / Windows 11", ip: "182.1.22.45", status: "Aktif Saat Ini" },
                       { date: "24 Mei, 08:15", device: "Safari / iPhone 15 Pro", ip: "182.1.22.45", status: "Berhasil" },
                       { date: "23 Mei, 19:45", device: "Chrome / Windows 11", ip: "182.1.22.10", status: "Berhasil" },
                    ].map((log, idx) => (
                       <div key={idx} className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                ...
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{log.date} • IP: {log.ip}</p>
                             </div>
                          </div>
                          <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border ${idx === 0 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                             {log.status}
                          </span>
                       </div>
                    ))}
