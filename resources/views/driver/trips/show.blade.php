    ...
    <div class="max-w-4xl mx-auto space-y-8 font-poppins pb-24 w-full max-w-full min-w-0">
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-4 flex items-center justify-between shadow-sm">
            <div class="flex items-center gap-3">
                <a href="{{ route('driver.trips.index') }}" class="p-2.5 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all">
                    <!-- Left Arrow SVG -->
            ...
            <div class="bg-blue-50 px-3.5 py-1.5 rounded-xl border border-blue-100">
                <p class="text-[10px] font-black text-blue-700 uppercase tracking-wider leading-none">TRP-{{ str_pad($trip->id, 3, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>

        <!-- Trip Summary Card -->
        <div class="bg-gradient-to-br from-[#0b1329] via-[#0d1b3e] to-[#070c1b] text-white rounded-[2rem] p-6 sm:p-8 shadow-xl border border-slate-800/80 relative overflow-hidden">
            <div class="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full -mr-10 -mt-10 blur-2xl"></div>
            
            <div class="relative z-10">
                <div class="flex flex-wrap items-center gap-2 mb-4">
                    <span class="px-2.5 py-1 rounded-xl text-[9px] font-black uppercase tracking-wider bg-orange-500/20 text-orange-400 border border-orange-500/20">
                        Shift {{ ucfirst($trip->jadwal->shift) }}
                    </span>
                    <span class="px-2.5 py-1 bg-white/10 text-white/80 rounded-xl text-[9px] font-black uppercase tracking-wider border border-white/5">
                        {{ $trip->jadwal->tanggal_keberangkatan->format('d M Y') }}
                    </span>
                    <span class="px-2.5 py-1 rounded-xl text-[9px] font-black uppercase tracking-wider bg-blue-500/20 text-blue-400 border border-blue-500/20">
                        {{ str_replace('_', ' ', $trip->status_trip) }}
                    </span>
                </div>

                <h2 class="text-lg sm:text-2xl font-black mb-6 leading-tight flex items-center gap-3 uppercase bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5">
                        ...
                        <div class="min-w-0">
                            <p class="text-[8px] font-black text-white/40 uppercase tracking-widest mb-0.5">Armada Kendaraan</p>
                            <p class="text-[10px] font-black text-white uppercase truncate">{{ $trip->armada->nama_mobil ?? '-' }} • {{ $trip->armada->nomor_plat ?? '-' }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 bg-white/5 p-4 rounded-2xl border border-white/5">
        ...
        <div class="space-y-6">
            @forelse($trip->detailTrips as $index => $dt)
                ...
                @endphp
                <div class="bg-white rounded-3xl border transition-all duration-300 relative {{ $isDroppedOff ? 'border-emerald-100 bg-slate-50/50' : 'border-slate-200 shadow-sm' }} w-full">
                    
                    <div class="p-6">
                        <div class="flex items-start justify-between gap-4 mb-6">
                            <div class="flex items-center gap-4 min-w-0">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xs font-black transition-all shrink-0 {{ $isPickedUp ? 'bg-emerald-100 text-emerald-700 shadow-inner' : 'bg-slate-900 text-white shadow-md' }}">
                                    {{ $index + 1 }}
                                </div>
                                <div class="min-w-0">
                                    <h4 class="text-sm font-black text-slate-900 leading-none mb-1.5 truncate">{{ $p->nama ?? 'No Name' }}</h4>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[9px] font-black text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-md shrink-0">{{ $b->jumlah_penumpang }} PAX</span>
                                        <span class="px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-wider border shrink-0 {{ ($isDroppedOff || $hasPelunasan) ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100' }}">
                            ...
                            <div class="relative">
                                ...
                                <div class="flex items-center justify-between mb-1">
                                    ...
                                    <button type="button"
                                            ...
                                            class="text-[9px] font-black text-blue-600 uppercase flex items-center gap-1 hover:underline active:scale-95">
                                        Peta Rute
                                    </button>
                                </div>
                                <p class="text-xs font-semibold text-slate-600 leading-normal italic">
                                    "{{ $b->alamat_jemput }}"
                                </p>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $p->no_hp) }}"
                               target="_blank"
                               class="flex items-center justify-center gap-2 py-3 bg-slate-50 hover:bg-slate-100 text-slate-900 rounded-xl transition-all text-[10px] font-black border border-slate-200 active:scale-[0.97]">
                                <!-- Phone/Chat SVG -->
                            ...
                            <div class="flex items-center justify-center py-3 rounded-xl border font-black text-[10px] text-center uppercase tracking-wider">
                                @if($isDroppedOff)
