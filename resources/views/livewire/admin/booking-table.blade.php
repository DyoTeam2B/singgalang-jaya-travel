<div class="space-y-6">
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:hidden">
            @forelse ($bookings as $b)
                ...
                <div class="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm p-6 space-y-4">
                    ...
                    <div class="flex items-start justify-between gap-2 border-b border-slate-100 pb-3">
                        <div>
                            ...
                            <div class="flex items-center gap-1.5 mt-1.5">
                                ...
                                <span class="text-[9px] font-black text-slate-900 bg-slate-100 px-1.5 py-0.5 rounded-md">{{ $b->jumlah_penumpang }} Orang</span>
                            </div>
                        </div>
                        <x-status-badge :status="$b->status_booking" class="text-[8px] uppercase tracking-wider py-1 px-2.5 rounded-lg" />
                    </div>

                    <!-- Details -->
                    <div class="grid grid-cols-2 gap-4 text-[10px]">
                        <div class="space-y-1">
                            <p class="font-black text-slate-400 uppercase tracking-wider">Perjalanan</p>
                            <p class="font-bold text-slate-900 leading-tight">{{ $b->jadwal->rute->asal ?? 'N/A' }} → {{ $b->jadwal->rute->tujuan ?? 'N/A' }}</p>
                            <p class="text-slate-500 font-medium">{{ $b->jadwal ? \Carbon\Carbon::parse($b->jadwal->tanggal_keberangkatan)->translatedFormat('d M Y') : 'N/A' }} ({{ $b->jadwal ? ucfirst($b->jadwal->shift) : 'N/A' }})</p>
                        </div>
                        <div class="space-y-1">
                            <p class="font-black text-slate-400 uppercase tracking-wider">Pembayaran</p>
                            ...
                            <p class="text-[9px] font-bold text-slate-400 mt-1">
                                @if ($hasConfirmSent)
                                    <span class="text-blue-600">WA: Sent</span>
                                @else
                                    <span class="text-slate-400">WA: Not Sent</span>
                                @endif
                            </p>
                        </div>
                    </div>
                    ...
                    <!-- Contact Info -->
                    <div class="flex flex-wrap gap-x-4 gap-y-1.5 pt-3 border-t border-slate-100 text-[10px] text-slate-500 font-medium">
                    ...
                    <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                        @if (in_array($b->status_booking, ['dikonfirmasi', 'assigned_to_trip']) && !$hasConfirmSent)
                            <button 
                                ...
                                class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[9px] font-black uppercase tracking-wider transition-all shadow-md active:scale-95 disabled:opacity-50"
                            >
                        ...
                        @if ($b->status_booking === 'menunggu_verifikasi' && $pembayaranTerakhir && $pembayaranTerakhir->status_pembayaran === 'menunggu')
                            <a 
                                ...
                                class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-[9px] font-black uppercase tracking-wider transition-all shadow-md active:scale-95"
                            >
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        @if ($b->status_booking === 'dikonfirmasi')
                            <a 
                                ...
                                class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[9px] font-black uppercase tracking-wider transition-all shadow-md active:scale-95"
                            >
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            @empty
                <div class="bg-white rounded-[2rem] border border-slate-200/60 p-8 text-center text-slate-400 font-bold text-xs">
        ...
        <!-- Pagination -->
        <div class="px-8 py-6 bg-white rounded-[2rem] border border-slate-200/60 shadow-sm">
            {{ $bookings->links() }}
        </div>
    </div>
</div>
