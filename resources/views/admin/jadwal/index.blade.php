...
@section('content')
    <div class="space-y-8">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <p class="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] mb-2">Manajemen Jadwal</p>
                <h1 class="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Jadwal Keberangkatan</h1>
                <p class="text-sm font-bold text-slate-400 mt-1">Atur rute, shift, jam keberangkatan, dan kapasitas penumpang harian.</p>
            </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm">
            <form method="GET" action="{{ route('admin.jadwal.index') }}" class="flex flex-wrap gap-4 items-center">
                ...
                @if($search)
                    <a href="{{ route('admin.jadwal.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-black uppercase tracking-widest px-6 py-3.5 rounded-xl transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse($jadwal as $item)
                @php
                    ...
                    // Check actual status dynamically
                    $displayStatus = $item->status_jadwal;
                    if ($item->status_jadwal === 'aktif' && $bookedSum >= $item->kuota) {
                        $displayStatus = 'penuh';
                    }
                @endphp
                <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-all">
                    <!-- Card Header -->
                    <div class="p-6 border-b border-slate-100 flex items-start justify-between gap-3">
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">JDW-{{ str_pad($item->id, 3, '0', STR_PAD_LEFT) }}</p>
                    ...
                    <div class="p-6 space-y-4 flex-1">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-slate-400">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest">Tanggal</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-slate-400">
                                ...
                                @endif
                                <span class="text-[10px] font-black uppercase tracking-widest">Shift</span>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-slate-400">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest">Berangkat</span>
                            </div>
                            <span class="text-sm font-bold text-slate-800">{{ $item->jam_berangkat->format('H:i') }} WIB</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2 text-slate-400">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest">Kapasitas</span>
                            </div>
                    ...
                    <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center gap-2">
                        ...
                        <a href="{{ route('admin.jadwal.edit', $item->id) }}"
                           class="flex-1 bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-colors shadow-sm">
                            <!-- Pencil Icon SVG -->
                        ...
                        <form action="{{ route('admin.jadwal.toggle', $item->id) }}" method="POST" class="flex-1">
                            ...
                            @if($item->status_jadwal === 'nonaktif')
                                <button type="submit"
                                        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all">
                            ...
                            @else
                                <button type="submit"
                                        class="w-full bg-rose-50 hover:bg-rose-100 text-rose-600 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <form action="{{ route('admin.jadwal.destroy', $item->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                    class="p-2.5 bg-white border border-slate-200 hover:bg-rose-50 hover:border-rose-200 text-slate-400 hover:text-rose-600 rounded-xl transition-colors shadow-sm"
                                    title="Hapus Jadwal">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            @empty
                <div class="col-span-full bg-white rounded-[2rem] border border-dashed border-slate-200 p-12 text-center">
                    ...
                    <p class="text-sm font-bold text-slate-500">Tidak ada jadwal ditemukan.</p>
                </div>
            @endforelse
        </div>
