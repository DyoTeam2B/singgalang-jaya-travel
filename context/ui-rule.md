...
| **Warning** | Booking Dibuat, Menunggu Verifikasi, Menunggu | `bg-yellow-100 text-yellow-800` |
...
```text
Home | Jadwal | Booking Saya | Charter | Kontak | Profil (Dropdown)
```
...
```
Booking Form → Review Booking → Upload Bukti DP → Menunggu Verifikasi → Dikonfirmasi
```

> ❌ Jangan skip step
> Pelanggan WAJIB login sebelum booking.
> DP flat Rp50.000 per booking.
> Jika DP ditolak, pelanggan upload ulang bukti pembayaran.
> Jika pelanggan membatalkan, DP hangus.
...
| 3 | Armada | `admin.armada.index` | Car |
| 4 | Booking | `admin.bookings.index` | Clipboard |
| 5 | Pembayaran | `admin.pembayaran.index` | CreditCard |
| 6 | Jadwal | `admin.jadwal.index` | Calendar |
| 7 | Trip | `admin.trips.index` | Truck |
| 8 | Driver | `admin.drivers.index` | Users |
| 9 | Laporan | `admin.laporan.index` | BarChart |
...
| Total Trip | Count semua trip |
| Total Pendapatan | Sum `total_harga` booking `completed` |
| Total Driver | Count semua driver |
| Total Armada | Count semua armada |
...
| 2 | Riwayat Trip | `driver.trips.index` |
...
- **Left**: Daftar penumpang (manifest) — status jemput/antar, alamat jemput, titik maps, sisa pembayaran
